...
namespace NHST.Controllers
{
    public class PayhelpController
    {
        ...
        public static List<Danhsachyeucau> GetByUserInSQLHelper_nottextnottypeWithstatus(int page, int maxrows, string s, int st, string fd, string td, int sort)
        {
            ...
            var sql = @"select p.ID, p.Username, p.TotalPrice, p.TotalPriceVND, p.Currency, p.Status, p.CreatedDate, p.IsNotComplete, p.FeeBuyPro, p.MaDonHang, p.SalesID, p.SalesName, ";
            ...
            while (reader.Read())
            {
                ...
                if (reader["FeeBuyPro"] != DBNull.Value)
                    entity.FeeBuyPro = Convert.ToDouble(reader["FeeBuyPro"]);
